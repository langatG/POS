...
Public Class frmStockAdjustment_Store_Record
        ...
        Try
            If dgw.Rows.Count > 0 Then
                If lblSet.Text = "SA" Then
                    ...
                    frmStockAdjustment_Store.txtBarcode.Text = dr.Cells(5).Value.ToString()
                    ...
                    End If
                    frmStockAdjustment_Store.txtQty.Text = dr.Cells(7).Value.ToString()
                    frmStockAdjustment_Store.txtQ.Text = dr.Cells(7).Value.ToString()
                    frmStockAdjustment_Store.txtReason.Text = dr.Cells(8).Value.ToString()
                    frmStockAdjustment_Store.txtBarcode.Enabled = False
    ...
    Private Sub txtBarcode_TextChanged(sender As System.Object, e As System.EventArgs) Handles txtBarcode.TextChanged
        Try
            con = New SqlConnection(cs)
            con.Open()
            cmd = New SqlCommand("SELECT  StockAdjustment_Store.SA_ID, StockAdjustment_Store.Date, StockAdjustment_Store.ProductID, RTRIM(Product.ProductCode),RTRIM(Product.ProductName), RTRIM(StockAdjustment_Store.Barcode), RTRIM(StockAdjustment_Store.AdjustmentType),StockAdjustment_Store.Qty, RTRIM(StockAdjustment_Store.Reason) FROM StockAdjustment_Store INNER JOIN Product ON StockAdjustment_Store.ProductID = Product.PID where StockAdjustment_Store.Barcode like '" & txtBarcode.Text & "%' order by Date", con)
            rdr = cmd.ExecuteReader(CommandBehavior.CloseConnection)
            dgw.Rows.Clear()
            While (rdr.Read() = True)
                dgw.Rows.Add(rdr(0), rdr(1), rdr(2), rdr(3), rdr(4), rdr(5), rdr(6), rdr(7), rdr(8))
