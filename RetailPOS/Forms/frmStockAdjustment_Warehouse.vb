...
Public Class frmStockAdjustment_Warehouse
    ...
    Sub GetQty_S()
        Try
            ...
            con.Open()
            cmd = con.CreateCommand()
            cmd.CommandText = "Select IsNull(Sum(Qty),0) from Temp_Stock,Product where Temp_Stock.ProductID=Product.PID and ProductID=@d1 and Temp_Stock.Barcode=@d2 group by ProductID,Temp_Stock.Barcode"
            cmd.Parameters.AddWithValue("@d1", Val(txtProductID.Text))
            cmd.Parameters.AddWithValue("@d2", txtBarcode.Text)
            rdr = cmd.ExecuteReader()
            If rdr.Read() Then
